...
json;
	...
	public function message_verify(){
		...
		$content="[学之友微额速达]您的验证码是{$verify}";//要发送的短信内容
...
json;
	...
	public function loan(){
		...
		if($is_adopt['is_success'] == 0){
			
			if(!empty($is_adopt) && in_array($is_adopt['is_adopt'],array(0,-1,1))){
			 $this->redirect('Weixin/examine');
			}
		}elseif($is_adopt['is_success'] == 2 && ($apply_time>time()) ){
	...
	public function fail_content(){
		$id = I('id');
		$fail_content = M('user_money_info')->where("id='$id'")->getField('fail_content');
...
json;
	...
	public function examine(){
		...
		$user_money_info = D('user_money_info');
		$money = $user_money_info->field('is_adopt')->where($map)->order("id desc")->select();
		
		$weixin_img = D('weixin_img');
		$payment_list = D('payment_list');
		if($money[0]['is_adopt'] == '0'){
			$img = $weixin_img->where($map)->select();
			$list = $payment_list->where($map)->select();
			...
			$bank_info = D('bank_info');
			$bank = $bank_info->field('is_payment')->where($map)->select();
			
			if($bank[0]['is_payment'] == '0'){
				$data = "2";
			}else if($bank[0]['is_payment'] == '1'){
				$res = $payment_list->where($map)->select();
				if($res[0]['wait_xuqi'] == '1'){
					$data = '6';  //续期中
				}else if($res[0]['wait_xuqi'] == '3'){
					$data = '7';  //还款中
				}else{
					$weixin = $weixin_img->where($map)->select();
					if($weixin){
						$data = '4';
					}else{
						$data = "5";
	...
	public function queren_loan(){
		...
		$user_info = D('user_info');
		$info = $user_info->where($map)->find();
		
		$bank_info = D('bank_info');
		$list = $bank_info->where($map)->select();
...
json;
	...
	public function info(){
		...
		$user_money_info = D('user_money_info');
		$money = $user_money_info->where("openid='$openid'")->order("id desc ")->select();
		...
		$fang = $payment_list->join("haoidcn_user_money_info as a ON haoidcn_payment_list.openid=a.openid")->where("haoidcn_payment_list.user_money_info=a.id")->find();

		$xuesheng = D('xuesheng');
		$xue = $xuesheng->where("openid='$openid'")->find();
	...
	public function loan_info(){
		...
		if($get_user[0]['is_loan'] == '1'){
			$payment = $payment_list->where("user_money_info_id='$user_money_info_id' and is_repayment=0")->find();
		...
		$text = <<<json
			{
			    ...
			    "news":{
			        "articles": [
			         {
			             "title":"交易提醒",
			             "description":"尊敬的用户，您{$result['apply_time']}在学之友微额速达借款{$result['money_num']}元{$result['time_length']}天，已成功还款.
交易时间：$time
交易类型：还款费用结算
交易金额：{$money}元
...
json;
	...
	public function zfb_jy(){
		...
		echo D('payment_list')->getLastSql();
		$data = "支付宝还款";
		$this->ajaxReturn($data);
	}
	...
	public function zfb_jy2(){
		...
		$payment_list = D('payment_list');
		$payment_list->where("user_money_info='$id'")->setField('trade_mode','2');
