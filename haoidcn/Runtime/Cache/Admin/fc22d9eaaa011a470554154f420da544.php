                        ...
                        <div>
                           <div class="widgetbox personal-information">
                               ...
                               <div class="widgetcontent dataTables_wrapper">
                               		<form id="form" action="<?php echo U('Task/examine_list');?>" method="post">
		                            	<input type="hidden" value="<?php echo ($type); ?>" id="type">
		                            	<select name="type" style="width: 80px;" id="select1">
		                            		<option value="1">手机号</option>
		                            		<option value="2">姓名</option>
		                            	</select>
		                            	<input type="text" class="input-xlarge" name="content" placeholder="请输入要搜索的手机号码或姓名" style="width:200px;" value="<?php echo ($content); ?>">
                                   ...
                                   <table class="table">
                                   	<tr>                                    	
                                        ...
                                        <th width="50px">初审状态</th>
                                   		...
                                   		<th width="50px">审核人员</th>
                                   		<th width="150px">操作</th>                                                                  		
                                   	</tr>
                                   	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>                                        	 
                                   			 <td width="150px"><?php echo ($vo["phone"]); ?></td>
                                   			 <td width="150px"><?php echo ($vo["name"]); ?></td>
	                                   		 <td width="150px"><?php echo ($vo["apply_time"]); ?></td>
	                                   		 <td width="100px"><?php echo ($vo["money_num"]); ?></td>
	                                   		 <td width="100px"><?php echo ($vo["time_length"]); ?></td>
	                                         <td width="150px">
	                                         	<?php if($vo["state"] == 0): ?>未审
	                                         	<?php else: ?>
	                                         ...
	                                         <td width="150px">
	                                         	<?php if($vo["is_payment"] == null): ?>未绑定
	                                         	<?php elseif($vo["is_payment"] == 0): ?>
	                                         		未验证
	                                         	<?php elseif($vo["is_payment"] == 1): ?>
	                                         		验证成功
	                                         	<?php elseif($vo["is_payment"] == 2): ?>
	                                         		验证失败<?php endif; ?>
	                                         </td>   
	                                         <td><?php echo ($vo["daozhang"]); ?></td>
	                                         <td><?php echo ($vo["bank_num"]); ?></td>
	                                         <td><?php echo ($vo["account_opening"]); ?></td>
	                                         <td><?php echo ($vo["loan_people"]); ?></td>              
	                                         <td>
	                                         	<a href="#myModal" role="button"  data-toggle="modal" onclick="loan(this)" id="<?php echo ($vo["phone"]); ?>">确定放款</a>
	                                         </td>
...
function is_loan(){
	...
	$.ajax({
		type:"POST",
		url:"<?php echo U('Task/loan_success');?>",
		data:{'phone':phone},
		success:function(data){
			if(data == "放款失败"){
				alert(data);
			}else{
				setTimeout(function(){window.location.href="<?php echo U('Transaction/loan_list');?>"},1000);
			}
		}
	})
}
function loan_fail(){
	var phone = $('#phone').val();
	window.location.href="/index.php/Task/fangkuanshibai?phone="+phone;
}
</script>
</html>