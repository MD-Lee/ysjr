                   ...
                   <div>
                        <div class="widgetbox personal-information">
                            ...
                            <div class="widgetcontent">
                            	<form id="form" action="<?php echo U('Transaction/no_repayment');?>" method="post">
                            	...
                            	<table class="table table-bordered">
                            		...
                            		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            				...
                            				<td><?php echo ($vo["payment_time"]); ?></td>
                            				<td><?php echo ($vo["appoint_time"]); ?></td>
                            				<td>
                            					<?php if($vo["actual_time"] == null): ?>未还款
                            					<?php else: ?>
                            						<?php echo ($vo["actual_time"]); endif; ?>
                            				</td>
                            				<td>
                            				<?php if($vo["trade_mode"] == 1): ?>银行卡还款
                            				<?php elseif($vo["trade_mode"] == 2): ?>
                            					支付宝还款
                            				<?php else: endif; ?>
                            				</td>
                            				<td>
                            				<?php if($vo["huankuan_type"] == 1): ?>全额还款
                            				<?php elseif($vo["huankuan_type"] == 2): ?>
                            					续期费付款
                            				<?php else: endif; ?>
                            				</td>
                            				<td><?php echo ($vo["loan_people"]); ?></td>
                            				<td>
                            				<?php if($vo["wait_xuqi"] == 2): ?>已收款
                            				<?php elseif($vo["is_repayment"] == 0): ?>
                           						<a href="#myModal" role="button" class="btn btn-default" data-toggle="modal" onclick="loan(this)" id="<?php echo ($vo["phone"]); ?>">审核</a>
                            				<?php elseif($vo["is_repayment"] == 1): ?>
                            					已还款<?php endif; ?>
                            				</td>
...
function is_Loan(){
	...
	$.ajax({
		type:"POST",
		url:"<?php echo U('Transaction/is_loan');?>",
		data:{'phone':phone},
		datatype:"jaon",
...
function no_Loan(){
	...
	$.ajax({
		type:"POST",
		url:"<?php echo U('Transaction/no_loan');?>",
		data:{'phone':phone},
		datatype:"jaon",
		success:function(data){
			setTimeout(function(){window.location.reload()},1000);
